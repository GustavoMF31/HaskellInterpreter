...
evaluate :: String -> Context -> Variable
evaluate expression context
    -- If it's an integer
    | isNumber expression = IntVar (read expression)
    -- If it's a float
    | ((length parts) == 2) && (isNumber p1) && (isNumber p2) = FloatVar (read expression) 
    -- If it's the name of a variable
    | M.member expression context = context M.! expression
    -- If it' a bool, return it
    | expression == "+" = BoolVar True
    | expression == "-" = BoolVar False
    -- If it doesn't look like an expression, it must be a long string!
    | ((length chunks) /= 3) || (operator `notElem` operators) = StrVar expression
